  ...
  const Cam = {
    ...
    async start(videoEl, canvasEl) {
      ...
      if (!this.landmarker) {
        ...
        const vision = await import(/* @vite-ignore */ C.TASKS_VISION_CDN); //güncellendi
        const files = await vision.FilesetResolver.forVisionTasks(
          C.TASKS_VISION_CDN + "/wasm",
        );
    ...
    _gozKutu(lm, idx, w, h) {
      const xs = idx.map((i) => lm[i].x * w),
        ys = idx.map((i) => lm[i].y * h);
      const x1 = Math.min(...xs),
        x2 = Math.max(...xs),
        y1 = Math.min(...ys),
        y2 = Math.max(...ys);
      const px = (x2 - x1) * 0.3,
        py = Math.max(y2 - y1, (x2 - x1) * 0.5) * 0.6;
      return [
        Math.max(x1 - px, 0),
        Math.max(y1 - py, 0),
        Math.min(x2 + px, w),
    ...
    _alarmBaslat() {
      ...
      if (s.sfx !== false && this.alarmAudio)
    ...
    _status(state, text, color, extra) {
      if (this.onStatus)
        this.onStatus(Object.assign({ state, text, color }, extra || {}));
    },
    ...
    _loop() {
      ...
      if (v.readyState >= 2 && this.landmarker) {
        ...
        if (sonuc.faceLandmarks && sonuc.faceLandmarks.length) {
          ...
          const earlar = [
            this._ear(lm, C.SOL_EAR, w, h),
            this._ear(lm, C.SAG_EAR, w, h),
          ].filter((e) => e !== null);
          const ear = earlar.length
            ? earlar.reduce((a, b) => a + b, 0) / earlar.length
            : null;
          const esik = yatik
            ? C.EAR_ESIK * C.EAR_ESIK_YATIK_CARPAN
            : C.EAR_ESIK;
          ...
          if (kapali) {
            ...
            if (sure >= C.KAPALI_SURE_ESIGI) {
              ...
              this._status(
                "alarm",
                `UYARI! ${reason} (${sure.toFixed(1)} sn) — UYAN!`,
                "red",
                { ear, aci, sure },
              );
            } else {
            ...
            this.uykuBaslangic = null;
            this._status("odak", "Gözler açık — odaklanmış", "green", {
              ear,
              aci,
            });
          }
          ...
          if (this.yuzGorulmus) {
            ...
            if (sure >= C.YUZ_YOK_ESIGI) {
              ...
              this._status(
                "alarm",
                `UYARI! Yüz görünmüyor (${sure.toFixed(0)} sn)`,
                "red",
                { sure },
              );
            } else {
              this._status(
                "yuz-yok",
                `Yüz görünmüyor (${sure.toFixed(1)} sn)`,
                "orange",
                { sure },
              );
            }
