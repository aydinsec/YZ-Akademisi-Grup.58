...
const DURATION_HINTS = [
  { re: /\b(tez|makale yaz|literatür|araştırma raporu)/i, min: 90 },
  { re: /\b(sunum|slayt|deck)/i, min: 60 },
  { re: /\b(rapor|doküman|dokümantasyon)/i, min: 60 },
  { re: /\b(etiketle|veri seti|veri temizle)/i, min: 60 },
  { re: /\b(model eğit|eğitim başlat|fine.?tun)/i, min: 90 },
  { re: /\b(kod yaz|geliştir|implement|refactor|hata ayıkla|debug)/i, min: 75 },
  { re: /\b(analiz|grafik|görselleştir)/i, min: 50 },
  { re: /\b(oku|incele|gözden geçir)/i, min: 40 },
  { re: /\b(liste hazırla|liste çıkar|malzeme listesi|planla)/i, min: 30 },
  { re: /\b(mail|e.?posta|yaz(ış)?ma|mesaj at|ara(mak)?|telefon)/i, min: 15 },
  { re: /\b(toplantı|görüşme|call)/i, min: 45 },
  { re: /\b(kargo|postane|eczane|market|alışveriş|fatura|banka)/i, min: 25 },
  { re: /\b(temizlik|topla|düzenle|bulaşık|çamaşır)/i, min: 35 },
  { re: /\b(spor|koş|yürüyüş|antrenman|egzersiz|yoga)/i, min: 45 },
  { re: /\b(doktor|randevu|hastane|tahlil)/i, min: 60 },
  { re: /\b(ders çalış|konu anlat|test çöz|soru çöz)/i, min: 50 },
];

/* Öncelik ipuçları */
const PRIO_HIGH = /\b(acil|bugün|hemen|son gün|deadline|teslim|yetiştir|kesin|mutlaka|ilk iş|önemli|unutma)/i;
const PRIO_LOW = /\b(müsait olursa|zamanım kalırsa|fırsat bulursam|bir ara|acele yok|istersem|opsiyonel)/i;

/* Zaman ipuçları */
const UPCOMING = /\b(yarın|haftaya|gelecek hafta|önümüzdeki|pazartesi|salı|çarşamba|perşembe|cuma|cumartesi|pazar|ayın \d+|\d+\s*(ocak|şubat|mart|nisan|mayıs|haziran|temmuz|ağustos|eylül|ekim|kasım|aralık))/i;
